...
if (isset($_GET["id"])) {
    ...
    if ($result && $result->num_rows > 0) {
        ...
        $insertSql = "INSERT INTO product_createtrash (ProductName, ProductType, Remarks, SalesPrice, ProductCost, Taxes, ReferenceNumber, Vendor, ItemsPurchased, IssuedProduct, Qtyonhand, Category) SELECT ProductName, ProductType, Remarks, SalesPrice, ProductCost, Taxes, ReferenceNumber, Vendor, ItemsPurchased, IssuedProduct, Qtyonhand, Category FROM product_create WHERE id = '$id'";
        $insertResult = $connection->query($insertSql);

        $deleteSql = "DELETE FROM product_create WHERE id = '$id'";
